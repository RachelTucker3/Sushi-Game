...
public class ChefImpl implements Chef, BeltObserver {
	...
	private double amount_consumed;
	private double amount_spoiled;
		...
		if (e.getType() == BeltEvent.EventType.PLATE_CONSUMED) {
			...
			if (plate.getChef() == this) {
				...
				for(int i = 0; i < plate.getContents().getIngredients().length; i++) {
					amount_consumed += plate.getContents().getIngredients()[i].getAmount();
				}
			...
			for(int i = 0; i < plate.getContents().getIngredients().length; i++) {
				amount_spoiled += plate.getContents().getIngredients()[i].getAmount();
			}
